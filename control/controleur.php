			...
			if ($verifTok == 1 /*and $verifIp == 1*/){
				...
				if($this->verifierAttributsJson($donnees, $attributsRequis)) {
					...
					if((new rdv)-> verifBan($id) == false){
						...
						if((new rdv)->exists($donnees->date, $donnees->heure, $donnees->idMedecin, $id)) {
							http_response_code(400);
							$renvoi = array("message" => "Impossible de prendre le rdv");
						}
						else {

							$resultat = (new rdv)->createRdv($donnees->date, $donnees->heure, $id, $donnees->idMedecin);
							if($resultat != false) {
								http_response_code(201);
			...
			if ($verifTok == 1 and $verifIp == 1){
				...
				if($this->verifierAttributsJson($donnees, $attributsRequis)) {
					if((new rdv)->exists($donnees->dateAct, $donnees->idMedecin)) {
						$resultat = (new rdv)->updateRdv($donnees->dateAct, $donnees->newDate, $donnees->newHeure, $donnees->heureAct, $id, $donnees->idMedecin);
	...
	public function supprimerRdv() {
		...
		if($donnees === null) {
			...
			if ($verifTok == 1 and $verifIp == 1){
				...
				if($this->verifierAttributsJson($donnees, $attributsRequis)) {
					if((new rdv)->exists($donnees->date, $donnees->heure, $donnees->idMedecin)) {
						$resultat = (new rdv)->deleteRdv($donnees->date, $donnees->heure, $id, $donnees->idMedecin);
